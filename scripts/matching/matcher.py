...
def gate2_token_jaccard(a: dict, b: dict, cfg: dict) -> GateResult:
    ...
    if (
        not cfg.get("consolidation", {})
        .get("gates", {})
        .get("gate2_token_jaccard", True)
    ):
        return GateResult.PASS
...
def gate3_exact_match(a: dict, b: dict, cfg: dict) -> GateResult:
    ...
    if canonical_tokens(a.get("name", "")) == canonical_tokens(b.get("name", "")):
        u1 = a.get("unit")
        u2 = b.get("unit")
        if units_value_compatible(
            u1, u2, cfg.get("consolidation", {}).get("unit_tolerance_pct", 15) / 100.0
        ):
...
def gate4_embedding(
    ...
    if model is not None and embeddings_a is not None and embeddings_b is not None:
        score = float(
            cosine_similarity([embeddings_a[idx_a]], [embeddings_b[idx_b]])[0, 0]
        )
    else:
        return GateResult.PASS
    ...
    if score >= auto_match:
        if units_value_compatible(
            a.get("unit"),
            b.get("unit"),
            cfg.get("consolidation", {}).get("unit_tolerance_pct", 15) / 100.0,
        ):
...
def gate5_price_plausibility(
    a: dict, b: dict, cfg: dict, effective_price_a: int = 0, effective_price_b: int = 0
) -> GateResult:
    """Gate 5: Flag for review if per-unit price ratio > max."""
    if (
        not cfg.get("consolidation", {})
        .get("gates", {})
        .get("gate5_price_plausibility", True)
    ):
        return GateResult.PASS
    if effective_price_a <= 0 or effective_price_b <= 0:
        return GateResult.PASS
    ratio_max = cfg.get("consolidation", {}).get("price_ratio_max", 3.0)
    ratio = max(effective_price_a, effective_price_b) / min(
        effective_price_a, effective_price_b
    )
    if ratio > ratio_max:
...
def _verify_one_pair(
    client, model_name: str, prompt: str, max_retries: int
) -> str | None:
...
def _gemini_verify(pairs: list[dict], cfg: dict) -> list[str | None]:
    ...
    api_key = cfg.get("ocr", {}).get("gemini", {}).get("api_key") or os.getenv(
        "GEMINI_API_KEY"
    )
    if not api_key:
    ...
    for p in pairs:
        prompt = AI_PROMPT_TEMPLATE.format(
            ...
            name_a=p["name_a"],
            unit_a=p.get("unit_a", ""),
            price_a=p.get("price_a", 0),
            store_b=p["store_b"],
            name_b=p["name_b"],
            unit_b=p.get("unit_b", ""),
            price_b=p.get("price_b", 0),
        )
        try:
...
def load_embedding_model(model_name: str):
    ...
    logger.info(
        "Loading embedding model '%s' (first run downloads ~90MB)...", model_name
    )
...
def match_products(
    ...
    emb_a = None
    emb_b = None
    if embedding_model is not None and cfg.get("consolidation", {}).get(
        "gates", {}
    ).get("gate4_embedding", True):
    ...
    for idx_a, prod_a in enumerate(lotte_products):
        ...
        for idx_b, prod_b in enumerate(superindo_products):
            ...
            # Gate 4
            r = gate4_embedding(
                prod_a, prod_b, cfg, embedding_model, emb_a, emb_b, idx_a, idx_b
            )
            if r == GateResult.MATCH:
                score = (
                    float(cosine_similarity([emb_a[idx_a]], [emb_b[idx_b]])[0, 0])
    ...
    superindo_only = [
        p for idx, p in enumerate(superindo_products) if idx not in matched_b_indices
    ]
    ...
    if ai_matched:
        ...
        logger.info(
            "Gate 6: sending %d ambiguous pairs to AI verifier...", len(ambiguous_pairs)
        )
