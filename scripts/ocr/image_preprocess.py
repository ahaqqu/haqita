...
def preprocess_for_ocr(img_path: str, cfg: dict) -> str:
    """
    Enhance image before OCR for Ollama provider.
    Returns original path if preprocessing is disabled or provider is not Ollama.
    """
    provider = cfg['ocr']['provider']

    # Preprocessing only for Ollama, and only if enabled in config
    if provider != 'ollama':
        return img_path

    ollama_cfg = cfg['ocr'].get('ollama', {})
    if not ollama_cfg.get('preprocess', True):
        return img_path
